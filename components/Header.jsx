...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-primary/10 bg-white backdrop-blur-md dark:bg-background-dark/80 transition-all duration-300">
      <nav
        aria-label="Global"
      ...
      >
        ...
        <div className="flex items-center gap-3">
          {user ? (
            <>
              <SesionPanel user={user} />
              <LogoutButton onClick={handleLogout} />
            </>
          ) : (
            <Link
              href="/login"
              className="text-sm font-semibold text-slate-700 hover:text-orange-500 px-3 py-2 rounded-md border border-transparent hover:border-orange-200 transition"
            >
              Iniciar sesión
            </Link>
          )}
